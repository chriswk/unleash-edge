...
impl FromRequest for EdgeToken {
    ...
    fn from_request(req: &HttpRequest, _payload: &mut Payload) -> Self::Future {
        let value: Option<EdgeResult<EdgeToken>> = req.headers().get("Authorization").map(|auth_header| {
            EdgeToken::try_from(auth_header.clone())
        });
...
impl TryFrom<HeaderValue> for EdgeToken {
    ...
    fn try_from(value: HeaderValue) -> Result<Self, Self::Error> {
        value.to_str().map_err(|_| EdgeError::InvalidHeaderValue).and_then(|t| EdgeToken::from_str(t))
    }
}
...
impl FromStr for EdgeToken {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(env_and_key) = token_parts.get(1) {
            let eAK: Vec<String> = env_and_key.split(".").take(2).map(|s| s.to_string()).collect();
            if eAK.len() != 2 {
                return Err(EdgeError::AuthorizationDenied)
            }
            Ok(EdgeToken {
                token: eAK.get(1).unwrap().clone(),
                environment: eAK.get(0).unwrap().clone(),
